...
/**
 * \file    billmysales/admin/setup.php
 * \ingroup billmysales
 * \brief   Billmysales setup page.
 */
...
require_once '../lib/billmysales.lib.php';
//require_once "../class/myclass.class.php";

// Translations
$langs->loadLangs(array("admin", "billmysales@billmysales"));
...
$arrayofparameters = array(
	'BILLMYSALES_WEBHOOK_URL'=>array('type'=>'string', 'css'=>'minwidth500' ,'enabled'=>1),
	'BILLMYSALES_WEBHOOK_TOKEN'=>array('type'=>'securekey','enabled'=>1),
	'BILLMYSALES_WEBHOOK_LOG'=>array('type'=>'yesno', 'enabled'=>1),
	//'BILLMYSALES_MYPARAM3'=>array('type'=>'category:'.Categorie::TYPE_CUSTOMER, 'enabled'=>1),
	//'BILLMYSALES_MYPARAM4'=>array('type'=>'emailtemplate:thirdparty', 'enabled'=>1),
	//'BILLMYSALES_MYPARAM5'=>array('type'=>'thirdparty_type', 'enabled'=>1),
	//'BILLMYSALES_MYPARAM6'=>array('type'=>'securekey', 'enabled'=>1),
	//'BILLMYSALES_MYPARAM7'=>array('type'=>'product', 'enabled'=>1),
);
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/billmysales/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
			$filefound = 1;
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'BILLMYSALES_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = 'BILLMYSALES_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
echo '<span class="opacitymedium">'.$langs->trans("BillmysalesSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'BILLMYSALES_'.strtoupper($myTmpObjectKey).'_ADDON';
